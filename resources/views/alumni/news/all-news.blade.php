            ...
            @forelse($newsData as $news)
                <div class="col-xl-3 col-md-6 col-sm-6 mb-4">
                    <div class="border-0 card news-card overflow-hidden rounded-4 shadow">
                        ...
                        <div class="card-body">
                            <h5 class="card-title text-secondary-color" style="color: #002a5c;">{{$news->title}}</h5>
                            <p class="card-text news-card-text fs-13" style="color: #002a5c;">{{ getSubText($news->details, 200)}}</p>
                            <a href="{{route('alumni.news.details',$news->slug)}}" class="bg-secondary-color border-0 btn btn-primary btn-read-more">Read More</a>
                        </div>
                    </div>
                </div>
            @empty
